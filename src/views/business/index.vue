	...
	<div class="business_main">
		<div style="margin-bottom: 20px;background: #fff;">
			<div class="category_title">
				<p class="font-title">类别检索</p>
			</div>
			<div class="category_to_retrieve">
				<div class="select_type">
					<el-select v-model="formInline.region" placeholder="申请人" size="medium">
						<el-option v-for="(item, index) in type" :key="index" :label="item.name" :value="item.id" />
					</el-select>
				</div>
				<div class="select_type_name">
					<el-input v-model="formInline.user" :placeholder="type_name" size="medium" clearable
						style="width: 300px" />
				</div>
			</div>
					...
					<div class="category_to_retrieve" style="padding: 0">
						<div class="select_type">
							<el-select v-model="formInline.proCode" placeholder="业务类型" size="medium">
								...
								<el-option v-for="(item, index) in proType" :key="index" :label="item.productionName"
									:value="item.productionCode" />
							</el-select>
						</div>
					</div>
				</div>
				<div class="select_date" style="padding-left: 0">
					...
					<div>
						<el-date-picker v-model="startDate" value-format="yyyy-MM-dd HH:mm:ss" type="datetime"
							size="medium" prefix-icon="el-icon-date" placeholder="选择开始时间" @change="getSStartTime" />
						<span class="date_section">—</span>
						<el-date-picker v-model="endDate" value-format="yyyy-MM-dd HH:mm:ss" type="datetime"
							size="medium" prefix-icon="el-icon-date" placeholder="选择结束时间" @change="getSEndTime" />
					</div>
			...
			<div class="allSelect_btn">
				<div class="button_set" style="margin-top: 15px;">
					<p style="padding: 0;font-weight: bolder;width: 100px;white-space: nowrap;" class="font-title">订单状态：
					</p>
					<div class="btn_box" style="min-width: 600px;">
						<div class="allBtn" style="margin-left: 0;">
							<el-checkbox-group v-model="testAll" @change="getValueAll()">
								<el-checkbox v-for="(item, index) in allBtnData" :key="index" :label="item.content"
									:checked="item.checked" />
							</el-checkbox-group>
						</div>
						<div>
							<el-checkbox-group ref="checkData" v-model="test" @change="getValue(test)">
								<el-checkbox v-for="(item, i) in items" :key="i" :label="item.content"
		...
		<div class="tableData_main">
			<div class="action_button">
				<el-tooltip class="item" effect="dark" placement="bottom">
					...
					<el-button size="medium" style="margin-bottom: 24px;color: #409EFF;border-color: #409EFF"
						@click="search(2)">导出订单</el-button>
				</el-tooltip>
				<div v-if="orgId!=17 && orgId!=3">
			...
			<el-table v-loading="loading" :data="tableData" border style="width: 100%" :row-class-name="rowClassName">
				...
				<el-table-column prop="applicantMobile" label="联系方式" align="center" show-overflow-tooltip />
				<el-table-column prop="createTime" label="订单提交时间" align="center" show-overflow-tooltip />
				...
				<!-- <el-table-column prop="orderSource" label="订单来源" align="center">
					<template slot-scope="scope">
						<span>{{ scope.row.orderPlatformSource | getSource }}</span>
					</template>
				</el-table-column> -->
				...
				<el-table-column label="操作" min-width="100" align="center">
					<template slot-scope="scope">
						<el-button size="mini" type="text" class="exit_btn" style="font-size: 14px"
...
export default {
	...
	data() {
		return {
			...
			items: [
				...
				// { id: 601, content: '待初审', disabled: false, checked: false },
				// { id: 601, content: '待复审', disabled: false, checked: false },
			],
			// 初始加载表格的页码
			currentPage3: 1,
			// 初始加载表格每页显示的数据
	...
	created() {
		...
		if (sessionStorage.getItem('pageNum')) {
			this.currentPage3 = parseInt(sessionStorage.getItem('pageNum'))
		}
	...
	methods: {
		...
		getList() {
			var data = { pageNum: this.currentPage3, pageSize: this.pageSize }
		...
		handleCurrentChange(val) {
			sessionStorage.setItem('pageNum', this.currentPage3)
			this.search()
		},
		reset() {
			this.test = []
