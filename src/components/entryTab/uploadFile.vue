      ...
      <!-- <div v-if="docInfoList.length == 0" class="no-list">
        <span>上传注意事项：当前仅能识别.pdf .doc
          .docx三种格式的文件，请把非以上三种文件的格式转换成任意之一的格式，再上传到系统之中。为了避免签名区域设置出错，请上传纸张大小为<span
            style="color: red"
          >A4</span>、或<span style="color: red">A3</span>的文件</span>
        <el-upload
          action
      ...
      <div class="have-list">
        ...
        <div class="doc-list">
          <div v-for="(i, index) in docInfoList" :key="'docInfo' + index">
            ...
            >正在编辑</span>
            <span
              v-if="editList < index && i.coverStatus !== 1"
              style="color: #909399"
            >未设置区域</span>
            <span>{{ i.docName }}</span>
...
import { checkRelationalPersonInformation } from '@/api/newhouse'
export default {
  props: {
    show: Boolean,
    ID: String,
      ...
      de.getSignDiv({ orderDocumentId: id }).then((res) => {
        if (res.success === true) {
          if (res.data != null || res.data.length !== 0) {
            ...
            // if (this.isSize === 2) {
            //   for (const i of list) {
            //     i.lowLeftX = parseInt(i.lowLeftX) / 0.69
            //     i.lowLeftY = parseInt(i.lowLeftY) + 40
            //   }
            // }
            this.signList = list
          }
    ...
    async addNowPage() {
      ...
      if (this.$parent.$parent.$parent.$parent.basicData.prodCode === 'ysf') {
        ...
        for (let index = 0; index < this.selectMan.length; index++) {
          arr.push(this.manList[this.selectMan[index]].proposerId)
        }
        ys = await checkRelationalPersonInformation(arr)
      }
