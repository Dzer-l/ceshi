				...
				<el-row :gutter="20">
					<el-col :span="6" style="width:430px;">
						<el-form-item label="运单号">
							<el-input placeholder="请输入运单号" v-model="preserveBusinessFormData.expressNo">
							</el-input>
						</el-form-item>
					</el-col>
					<!-- <el-col :span="6" style="width:430px;">
						<el-form-item label="订单编号">
							<el-input placeholder="请输入订单编号" v-model="preserveBusinessFormData.orderNum">
							</el-input>
						</el-form-item>
					</el-col> -->
					<el-col :span="6" style="width:430px;">
						<el-form-item label="寄件人">
							<el-input placeholder="请输入寄件人" v-model="preserveBusinessFormData.senderName">
							</el-input>
						</el-form-item>
					</el-col>
				</el-row>
				...
				<el-row :gutter="20">
					<el-col :span="6" style="width:430px;">
						<el-form-item label="收件人姓名">
							<el-input placeholder="请输入收件人姓名" v-model="preserveBusinessFormData.receiveName">
							</el-input>
						</el-form-item>
					</el-col>
					<el-col :span="6" style="width:430px;">
						<el-form-item label="操作人">
							<el-select style="width:100%" v-model="preserveBusinessFormData.operatingUserName"
								placeholder="请选择操作人">
					...
					<el-col :span="6" style="width:430px;">
						<el-form-item label="快递状态">
							<el-select style="width:100%" v-model="preserveBusinessFormData.status"
								placeholder="请选择快递状态">
				...
				<el-row :gutter="20">
					<el-col style="width:430px;" :span="6">
						<el-form-item label="业务类型">
							<el-select style="width:100%" v-model="preserveBusinessFormData.prodCode"
								placeholder="请选择业务类型">
					...
					<el-col :span="8" style="width:540px;">
						<el-form-item label="寄件时间">
							<el-date-picker v-model="preserveBusinessFormData.startTime"
							...
							<el-date-picker v-model="preserveBusinessFormData.endTime"
								value-format="yyyy-MM-dd HH:mm:ss" type="datetime" prefix-icon="el-icon-date"
		...
		<div class="content_box">
			...
			<div class="pagination_content">
				<el-pagination @size-change="handleSizeChange" :page-sizes="[10, 20, 30, 40]" background
					:current-page.sync="preserveBusinessPagination.pageNum" @current-change="handleCurrentChange"
...
export default {
	...
	data() {
		return {
			...
			pickerStartOptions: {	// 开始时间
				disabledDate: (time) => {
					return time.getTime() > moment(this.preserveBusinessFormData.endTime).format("x")
				}
			},
			pickerEndOptions: {	// 结束时间
				disabledDate: (time) => {
					return time.getTime() < moment(this.preserveBusinessFormData.startTime).format("x")
				}
			},
			isEvid: 1,	// 0 公证业务 1 存证业务
		}
	},
	watch: {
		preserveBusinessFormData: {	// 搜索
			handler(val) {
				this.$emit('update:preserveBusinessFormData', val)
			},
			deep: true
		},
		preserveBusinessPagination: {  // 分页
			handler(val) {
				this.$emit('update:preserveBusinessPagination', val)
			},
			deep: true
		},
	},
	methods: {
		...
		// 查询
		handleSearch() {
			this.preserveBusinessPagination.pageNum = 1
			this.getList()
		},
		...
		async handleImportRecord() {
			...
			let fileName = this.sendReceiveType == 0 ? `寄件记录-存证业务快递订单 ${date}.xls` : `收件记录-存证业务快递订单 ${date}.xls`
		...
		handleSizeChange(val) {
			console.log('每页:' + val);
			this.preserveBusinessPagination.pageSize = val
			this.getList()
		},
		handleCurrentChange(val) {
			console.log('页面:' + val);
			this.preserveBusinessPagination.pageNum = val
			this.getList()
		},
	},
	created() {
