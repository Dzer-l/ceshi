<template>
	<div class="document-material-main">
		<div class="action-bar">
			<!-- 待审核状态显示的按钮-->
			<el-row v-if="staData.orderStatus === 201||staData.orderStatus === 306">
				<el-button v-if="staData.orderStatus != 306" type="primary" size="medium" :disabled="jurisdiction"
					@click="allSetDistrict">设置签名区域
				</el-button>
				<el-button type="primary" size="medium" :disabled="jurisdiction" @click="uploadMaterial">上传签署材料
				</el-button>
				<el-button type="primary" size="medium" :disabled="jurisdiction" @click="allDownload">全部下载</el-button>
			</el-row>
			<!-- 需补材料状态显示的按钮 -->
			<!-- <el-row>
				<el-button type="primary" size="medium" :disabled="jurisdiction" @click="allDownload">全部下载</el-button>
        ...
        ">
				<el-button type="primary" size="medium" @click="previewAllMaterial">预览</el-button>
				<el-button type="primary" size="medium" :disabled="jurisdiction" @click="allDownload">全部下载</el-button>
			</el-row> -->
		</div>
		<div class="document-material-table">
			<el-table :data="documentMaterialTableData" border style="width: 100%"
				...
				<el-table-column prop="coverStatus" label="签署区域设置状态" align="center">
					<template slot-scope="scope">
						<el-tag v-if="scope.row.coverStatus === 0" type="danger">未设置</el-tag>
						<el-tag v-if="scope.row.coverStatus === 1" type="success">已设置</el-tag>
					</template>
				</el-table-column>
				<el-table-column prop="" label="签署状态" align="center">
					<template slot-scope="scope">
						<el-tag v-if="scope.row.signStatus === 2" type="success">已签署</el-tag>
						<el-tag v-if="scope.row.signStatus === 0" type="danger">未签署</el-tag>
						<el-tag v-if="scope.row.signStatus === 1" type="warning">部分签署</el-tag>
					</template>
				</el-table-column>
				<el-table-column prop="" label="操作" fixed="right" width="350" align="center">
					<template slot-scope="scope">
						...
						<span style="margin-right:10px;">
							<el-button :disabled="jurisdiction" type="text" @click="previewFile(scope.row)">预览
							</el-button>
							<!-- <el-button v-if="getFileType(scope.row.originalFilename) === 'word'" type="text"
								:disabled="jurisdiction" @click="editDocument(scope.row)">编辑</el-button> -->
							<el-button v-if="scope.row.coverStatus === 0&&staData.orderStatus === 201"
								:disabled="jurisdiction" type="text" @click="setSignature(scope.row)">设置签名区域</el-button>
							<el-button
								v-if="scope.row.documentType!=900&&scope.row.coverStatus === 1&&(staData.orderStatus === 201||staData.orderStatus === 306)"
								type="text" :disabled="jurisdiction" @click="setSignature(scope.row)">重设签名区域</el-button>
							<el-button :disabled="jurisdiction" type="text" @click="printFile(scope.row)">打印
							</el-button>
							<!-- <el-button type="text" :disabled="jurisdiction" style="margin-left:5px"
								@click="singleDownload(scope.row)">下载</el-button> -->
							<el-button v-if="staData.orderStatus === 201||staData.orderStatus === 306" type="text"
								:disabled="jurisdiction" @click="deleteMaterials(scope.row)">删除
							</el-button>
						</span>
					</template>
				</el-table-column>
			...
			<!-- <div class="paging_box">
				<div class="paging">
					<el-pagination background :current-page.sync="currentPage3" :page-size="pageSize"
						layout="prev, pager, next,sizes, jumper" :total="total" @size-change="handleSizeChange"
						@current-change="handleCurrentChange" />
				</div>
			</div> -->
		</div>
		<sign-file :man-list="fileSignList" :i-d="orderId" :doc-info="docInfo" :show="signBoxShow" @sucSign="init" />

		<!-- 上传申请材料 -->
		<div class="upload-material-dialog">
			<el-dialog title="上传签署材料" :visible.sync="uploadMaterialDialogVisible" width="800px"
				:close-on-click-modal="false">
				<!-- <el-form label-width="80px">
					<el-form-item label="文件上传">
						<el-upload action :show-file-list="false" :multiple="true" :http-request="upLoadFile"
							:data="{ type: 'all' }">
							<el-button icon="el-icon-upload2">上传文件</el-button>

							<div slot="tip" class="el-upload__tip" style="margin-right: 50%">
								<span style="color: #999">支持扩展名：.doc .docx .pdf格式，请将上传的文件转成所支持的格式</span>
							</div>
						</el-upload>
				...
				<div class="upload_container">
					...
					<div slot="tip" class="el-upload__tip" style="margin-right: 50%">
						...
						<div class="el-upload__tip">
							<span style="color: #999">注意：.doc.docx.pdf格式，为了避免签名区域设置出错，请上传纸张大小为A4或A3的文件</span>
						</div>
					</el-upload>
				</div>
...
import {
	getUniversalDocument,		// 文书材料查询
	getUniversalOrderDetail,	// 获取订单详情
	DeleteWritMaterials
} from '../../api/universal'
import { printInit, print } from '../../utils/printDocuments';
import signFile from '../../components/universal/uploadFile.vue';
...
export default {
	...
	methods: {
		...
		getList(data) {
			// 新的
			getUniversalDocument(data).then((res) => {
				// console.log(res, '文书材料信息')
		...
		async uploadMaterial() {
			if (this.staData.orderStatus == 306) {
				let res = await this.handleComfirm("当前订单已完成，重新上传签署文件，将清空所有已签署文件的签名，需要重新签署，确定继续操作吗？").catch(err => { })
				if (!res) return
			}
		...
		// 提示
		handleComfirm(message) {
			return new Promise((resolve, reject) => {
				this.$confirm(`<p style="color:red;">${message}</p>`,
					'操作提示', {
		...
		async deleteMaterials(val) {
			if (this.staData.orderStatus == 306) {
				let res = await this.handleComfirm("当前订单已完成，删除文件，将清空所有已签署文件的签名，需要重新签署，确定继续操作吗？").catch(err => { })
				if (!res) return
			...
			this.$confirm('<p style="color:red;">删除后，文件不可恢复！</p>', '提示', {
		...
		allDownload() {
			if (this.documentMaterialTableData.length > 0) {
				...
				// var fileName = `${this.staData.businessTypeDesc}${this.staData.orderNum} ${this.getTime()}.zip`
				var fileName = `文书材料 ${this.orderNum}.zip`
		...
		// 打印
		printFile(row) {
			this.$axios(`org/acceptance-order-document-preview/${row.id}`).then(async res => {
				if (res.data.success) {
					const { ids } = res.data.data
					let images = []
					for (const i in ids) {
						const imgUrl = await this.getImg(ids[i], 1)
						images.push(imgUrl)
					}
					// console.log(images);
					await printInit(images)
					print()
					return
				}
		...
		previewFile(e) {
			this.$axios(`org/acceptance-order-document-preview/${e.id}`).then(async res => {
				showLoading()
		...
		async getImg(id, type) {
			const url =
				type === 1
					? `org/acceptance-order-document-preview-img/${id}`
					: `org/acceptance-order-proposer-material-preview-img?id=${id}`
		...
		async setSignature(e) {
			console.log("setSignature", e);
			if (this.staData.orderStatus == 306) {
				let res = await this.handleComfirm("当前订单已完成，重设已签署文件的签名，将清空所有已签署文件的签名，需要重新签署，确定继续操作吗？").catch(err => { })
				if (!res) return
			}
			de.getEntrustInfo({ orderId: this.orderId }).then(async (res) => {
				console.log('setSignature', res)
				if (res.success) {
					const flieList = []
					const fileSignList = []
					...
					this.docInfo = {
						...
						coverStatus: e.coverStatus
					}
				}
			})
		},

		async allSetDistrict() {
			// console.log(this.documentMaterialTableData);
			if (this.staData.orderStatus == 306) {
				let res = await this.handleComfirm("当前订单已完成，重设已签署文件的签名，将清空所有已签署文件的签名，需要重新签署，确定继续操作吗？").catch(err => { })
				if (!res) return
		...
		// 文件多选上传
		upLoadFile(e) {
			console.log(this.getFileType(e.file.name))
			...
			this.type = dataType
			console.log(this.upFileList, '------------------------------0000000000000---------------------------')
		...
		affirmUploading() {
			if (this.fileShow.length > 0) {
				...
				formData.append('orderId', this.orderId)
				this.$axios('org/acceptance-order-document', formData, {
					method: 'post',
					headers: { 'Content-Type': 'application/json' }
				}).then((res) => {
					console.log(res, '上传成功后的数据')
	...
	watch: {
		"fileShow.length"() {
			this.fileShow.length && this.affirmUploading()
			this.$emit("updateData")
		}
	},
}
