			...
			<div class="have-list">
				...
				<div class="doc-list">
					<div v-for="(i, index) in docInfoList" :key="'docInfo' + index">
						<span v-if="
                editList > index || (i.coverStatus === 1 && editList !== index)
              " style="color: #67c23a">已设置区域</span>
            ...
            : "保存当前文书设置，并进入下一份"
				...
				<div ref="signFileBox" class="sign-box" :style="{ width: 590 * isSize + 'px' }">
					...
					<template v-for="(man, i) in signList">
						<span v-if="man.page == editIndex + 1" :key="'man' + i" class="sign-area" :style="{
	...
	methods: {
		...
		getDocImg(id) {
			return new Promise(async (resolve, reject) => {
				await getUniversalDocumentPreviewId({ id: id }).then(async (res) => {
		...
		async getImg(id) {
			return await this.$axios(`org/acceptance-order-document-preview-img/${id}`).then(
				async (res) => {
					if (res.success) {
		...
		onClick_submitSign() {
			...
			// if (this.isSize === 2) {
			//   for (const i of orderProposerDocumentSigns) {
			//     i.lowLeftX = i.lowLeftX - (i.lowLeftX * 0.31)
			//     i.lowLeftY = i.lowLeftY - 40
			//   }
			// }
